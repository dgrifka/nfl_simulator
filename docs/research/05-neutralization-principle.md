...
| **Dropped pick** (interceptable throw, 2022+) | **Pass** — the ball is in the air and the defender's hands are partly outside the offence | `sigma_d` 0.254 | 22 / defence-season | — | **variant (no branch point dispute; A-3 pending).** A default-off component neutralizes each charted interception-worthy throw at the defence-season's posterior-sampled catch probability, **beside v1.3 and never in it**. Not a treatment: whether it becomes a row is amendment A-3, the maintainer's decision | 49, 50 |
