...
| **Dropped pick** (interceptable throw, 2022+) | **Pass** — the ball is in the air and the defender's hands are partly outside the offence | `sigma_d` 0.258 | 22 / defence-season | — | **partial (A-3).** Each charted interception-worthy throw is neutralized at the **defence-season's** posterior-sampled catch probability. It ships in the **Full** edition (2022–2025) only; the **Strict** edition (2016–2025) is v1.3 unchanged. A-3's three gates pass: G-1 self-fulfilment (bucket agreement 0.997, median \|ΔDTW\| between the in-sample and week-out arms 0.05 pp, and production keeps the in-sample read), G-2 pricing (0.94), G-3 materiality (1.62 pp against a 0.56 pp floor). Enacted 2026-08-28 by ruling R-4 | 49, 50, 52, 53, 54, 55, 58, 59 |
...
| **Drops** (catchable ball, 2022+) | **Fail** — a receiver's hands, not a coin | `sigma` 0.63 pp | 437 / team-season | — | **partial (A-3).** Gate A's answer is unchanged and document 09's skill finding stands: the team-season spread is **0.709 pp / 14.3% relative** and Gate C-2 **fails** on the conditioned residual, so receiving corps genuinely differ. A-3 is the second door beside Gate A: each charted catchable target is neutralized at the **team-season's** — the receiving corps' — posterior-sampled catch probability, in the **Full** edition only. Its four gates pass — G-4a (C-3 0.877 at the charged grain), G-4b (V-1..V-8), G-4c (agreement 0.996 over nineteen folds), G-4d (2.32 pp against a 0.56 pp floor). The charged grain is the corps, not the receiver: C-3 power at the individual grain is 0.400 and carries no verdict. **Charting caveat (S-2, document 59 §3):** a contested target is charted catchable **66.8%** of the time against **74.4%** uncontested (ratio 0.90×), so contest is associated with the charter's judgement — but document 58 §3's pre-registered trigger, the incompletion split, runs the *other* way (contested incompletions are charted uncatchable 62.3% against 81.1%). The evidence that `is_catchable_ball` encodes the **outcome** is therefore open rather than measured, so the 0.709 pp spread is reported with this caveat attached and not as a floor | 09 §2, §8; 56, 57, 58, 59 |
...
1. **No branch point** (penalties, returns, drops, fourth down, two point, all
   ...
   these were settled by mechanism before a model was fit. *Drops still fail
   Gate A and always will;* what changed on 2026-08-28 is that amendment A-3
   opened a second door beside Gate A, and the drop row now enters the **Full**
   edition through that door rather than through this one.
