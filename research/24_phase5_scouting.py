...
patterns = ["out_of_bounds", "own_kickoff", "onside", "pass_defense", "return_yard",
            "tip", "deflect", "batted", "overtime", "coin"]
...
print(f"    desc mentions tipped/deflected/batted: {tipped.height:,} "
...
if matched.height:
    ...
    print(f"    INTs on NON-interception-worthy throws: {unworthy.height:,} "
          f"({100*unworthy.height/matched.height:.1f}%)  <- deflection-channel candidates")
    print(f"    mean EPA of those plays (offense view): "
...
if "fumble_out_of_bounds" in pbp.columns:
    ...
    lost_rate = live.filter(pl.col("fumble_recovery_1_team") != pl.col("fumbled_1_team")).height / max(live.height, 1)
    print(f"    live fumbles lost by fumbling team: {100*lost_rate:.1f}% "
          f"(OOB fumbles are retained 100% by rule)")
    print(f"    OOB share per season: ",
          oob.group_by("season").len().sort("season")["len"].to_list())
else:
...
first_pos = (
    ...
    .agg(pl.col("posteam").first().alias("ot_first_posteam"),
         pl.col("home_team").first(), pl.col("result").first(),
         pl.col("season").first(), pl.col("season_type").first())
)
first_pos = first_pos.with_columns(
    winner=pl.when(pl.col("result") > 0).then(pl.col("home_team"))
    .when(pl.col("result") < 0).then(None)  # away wins handled below
...
if "own_kickoff_recovery" in pbp.columns:
    ...
    print(f"    recovered by kicking team: {rec.height} ({100*rec.height/max(onside.height,1):.1f}%)")
    okr_total = kicks.filter(pl.col("own_kickoff_recovery") == 1).height
    print(f"    own_kickoff_recovery flag on ALL kickoffs: {okr_total} "
...
print(f"    games with >=1 INT return: {per_game.height:,}; "
...
for v in [0.0, 0.05, 0.10, 0.20]:
    ...
    for _ in range(500):
        ...
        h1 = np.array([rng.normal(m, np.sqrt(1 - v) * sd_y, max(n // 2, 1)).mean()
                       for m, n in zip(team_mu, ns)])
        h2 = np.array([rng.normal(m, np.sqrt(1 - v) * sd_y, max(n - n // 2, 1)).mean()
                       for m, n in zip(team_mu, ns)])
        rs.append(np.corrcoef(h1, h2)[0, 1])
    rs = np.array(rs)
    print(f"    true variance share {v:.0%}: split-half r median {np.median(rs):+.3f}, "
